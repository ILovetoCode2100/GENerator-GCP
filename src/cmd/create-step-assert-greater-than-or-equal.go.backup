...
func newCreateStepAssertGreaterThanOrEqualCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "create-step-assert-greater-than-or-equal ELEMENT VALUE [POSITION]",
...
Examples:
  ...
  api-cli create-step-assert-greater-than-or-equal "Score" "75"          # Auto-increment position
  api-cli create-step-assert-greater-than-or-equal "#score-value" "80" 2         # Explicit position
  
  # Override checkpoint for specific step
  api-cli create-step-assert-greater-than-or-equal "Score" "75" 1 --checkpoint 1678319`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			stepID, err := client.CreateAssertGreaterThanOrEqualStep(ctx.CheckpointID, element, value, ctx.Position)
			if err != nil {
			...
			output := &StepOutput{
				...
				ParsedStep:   fmt.Sprintf("expect %s to be greater than or equal to \"%s\"", element, value),
