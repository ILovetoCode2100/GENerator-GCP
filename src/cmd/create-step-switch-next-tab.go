...
Example:
  ...
  api-cli create-step-switch-next-tab 1678318 2 -o json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			stepID, err := client.CreateSwitchNextTabStep(checkpointID, position)
			if err != nil {
			...
			case "json":
				output := map[string]interface{}{
					...
					"step_id":       stepID,
					"position":      position,
					"parsed_step":   "switch to next tab",
				}
			...
			case "yaml":
				...
				fmt.Printf("parsed_step: switch to next tab\n")
			case "ai":
				fmt.Printf("Successfully created switch next tab step:\n")
				fmt.Printf("- Step ID: %d\n", stepID)
				fmt.Printf("- Step Type: SWITCH (next tab)\n")
				fmt.Printf("- Checkpoint ID: %d\n", checkpointID)
				fmt.Printf("- Position: %d\n", position)
				fmt.Printf("- Parsed Step: switch to next tab\n")
				fmt.Printf("\nNext steps:\n")
				fmt.Printf("1. Add another step: api-cli create-step-* %d <options>\n", checkpointID)
				fmt.Printf("2. Switch back to previous tab: api-cli create-step-switch-prev-tab %d <position>\n", checkpointID)
				fmt.Printf("3. Execute the test journey\n")
			default: // human
				fmt.Printf("✅ Created switch next tab step at position %d in checkpoint %d\n", position, checkpointID)
				fmt.Printf("   Step ID: %d\n", stepID)
			}
