...
func newCreateStepScrollElementCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "create-step-scroll-element CHECKPOINT_ID ELEMENT POSITION",
...
Example:
  ...
  api-cli create-step-scroll-element 1678318 "#footer" 2 -o json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "json":
				output := map[string]interface{}{
					"status":        "success",
					"step_type":     "SCROLL_ELEMENT",
					"checkpoint_id": checkpointID,
					"step_id":       stepID,
					"element":       element,
					"position":      position,
					"parsed_step":   fmt.Sprintf("scroll to %s", element),
				}
			...
			case "yaml":
				...
				fmt.Printf("step_type: SCROLL_ELEMENT\n")
				fmt.Printf("checkpoint_id: %d\n", checkpointID)
				fmt.Printf("step_id: %d\n", stepID)
				fmt.Printf("element: %s\n", element)
				fmt.Printf("position: %d\n", position)
				fmt.Printf("parsed_step: scroll to %s\n", element)
			case "ai":
				fmt.Printf("Successfully created scroll to element step:\n")
				fmt.Printf("- Step ID: %d\n", stepID)
				fmt.Printf("- Step Type: SCROLL_ELEMENT\n")
				fmt.Printf("- Checkpoint ID: %d\n", checkpointID)
				fmt.Printf("- Element: %s\n", element)
				fmt.Printf("- Position: %d\n", position)
				fmt.Printf("- Parsed Step: scroll to %s\n", element)
				fmt.Printf("\nNext steps:\n")
				fmt.Printf("1. Add another step: api-cli create-step-* %d <options>\n", checkpointID)
				fmt.Printf("2. Execute the test journey\n")
			default: // human
				fmt.Printf("✅ Created scroll to element step at position %d in checkpoint %d\n", position, checkpointID)
				fmt.Printf("   Element: %s\n", element)
				fmt.Printf("   Step ID: %d\n", stepID)
			}
