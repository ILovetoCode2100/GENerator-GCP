...
// newWaitV2Cmd creates the new wait command using BaseCommand pattern
func newWaitV2Cmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "wait",
...
func (wc *WaitCommand) createWaitStep(stepType string, args []string, timeout int) (*StepResult, error) {
	...
	case "WAIT_ELEMENT":
		if timeout > 0 {
			...
			stepID, err = wc.Client.CreateStepWaitForElementTimeout(checkpointID, args[0], timeout, position)
		} else {
			// Use default timeout version
			stepID, err = wc.Client.CreateWaitElementStep(checkpointID, args[0], position)
		}
	case "WAIT_ELEMENT_NOT_VISIBLE":
		stepID, err = wc.Client.CreateStepWaitForElementNotVisible(checkpointID, args[0], timeout, position)
	case "WAIT_TIME":
