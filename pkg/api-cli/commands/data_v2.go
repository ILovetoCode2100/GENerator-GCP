...
// newDataV2Cmd creates the new data command using BaseCommand pattern
func newDataV2Cmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "data",
...
func (dc *DataCommand) createDataStep(stepType string, meta map[string]interface{}) (*StepResult, error) {
	...
	switch operation {
	case "ELEMENT_TEXT":
		stepID, err = dc.Client.CreateStoreStep(checkpointID,
			meta["selector"].(string),
			meta["variable"].(string),
			dc.Position)
	case "LITERAL":
		stepID, err = dc.Client.CreateStoreValueStep(checkpointID,
			meta["value"].(string),
			meta["variable"].(string),
			dc.Position)
	case "ATTRIBUTE":
		stepID, err = dc.Client.CreateStepStoreAttribute(checkpointID,
			meta["selector"].(string),
	...
	case "COOKIE_CREATE":
		...
		if len(options) > 0 {
			stepID, err = dc.Client.CreateStepCookieCreateWithOptions(checkpointID,
				meta["name"].(string),
				meta["value"].(string),
				options,
				dc.Position)
		} else {
			stepID, err = dc.Client.CreateStepCookieCreate(checkpointID,
				meta["name"].(string),
				meta["value"].(string),
				dc.Position)
		}
	case "COOKIE_DELETE":
		stepID, err = dc.Client.CreateDeleteCookieStep(checkpointID,
			meta["name"].(string),
			dc.Position)
	case "COOKIE_CLEAR_ALL":
		stepID, err = dc.Client.CreateClearCookiesStep(checkpointID, dc.Position)
	default:
