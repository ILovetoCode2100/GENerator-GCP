...
test_cmd "Scroll by negative" "$CLI navigate scroll-by $CHECKPOINT_ID $((POS++)) --x -100 --y -200"
...
echo "=================================="
test_cmd "Store element text" "$CLI data store element-text 'h1' 'pageTitle' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Store literal" "$CLI data store literal 'testValue123' 'myVar' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Store attribute" "$CLI data store attribute 'a.link' 'href' 'linkUrl' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Store src attribute" "$CLI data store attribute 'img' 'src' 'imageSrc' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Cookie create basic" "$CLI data cookie create 'session' 'abc123' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Cookie with domain" "$CLI data cookie create 'auth' 'xyz789' $((POS++)) --checkpoint $CHECKPOINT_ID --domain '.example.com'"
test_cmd "Cookie with path" "$CLI data cookie create 'prefs' 'settings1' $((POS++)) --checkpoint $CHECKPOINT_ID --path '/admin'"
test_cmd "Cookie secure" "$CLI data cookie create 'secure_token' 'secret' $((POS++)) --checkpoint $CHECKPOINT_ID --secure"
test_cmd "Cookie httpOnly" "$CLI data cookie create 'http_only' 'value' $((POS++)) --checkpoint $CHECKPOINT_ID --http-only"
test_cmd "Cookie all options" "$CLI data cookie create 'full' 'complex' $((POS++)) --checkpoint $CHECKPOINT_ID --domain '.test.com' --path '/' --secure --http-only"
test_cmd "Cookie delete" "$CLI data cookie delete 'session' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Cookie clear all" "$CLI data cookie clear-all $((POS++)) --checkpoint $CHECKPOINT_ID"
echo ""
...
test_cmd "Wait time 1s" "$CLI wait time 1000 --checkpoint $CHECKPOINT_ID"
test_cmd "Wait time 500ms" "$CLI wait time 500 --checkpoint $CHECKPOINT_ID"
echo ""

# 8. Test ALL Window Commands (Stage 1 & 3 Enhanced)
echo -e "${YELLOW}8. Window Commands (All Operations)${NC}"
echo "==================================="
test_cmd "Window resize" "$CLI window resize 1024x768 $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Window resize mobile" "$CLI window resize 375x667 $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Window maximize" "$CLI window maximize $((POS++)) --checkpoint $CHECKPOINT_ID"
...
test_cmd "Switch frame by name" "$CLI window switch frame-name 'contentFrame' $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Switch main content" "$CLI window switch main-content $((POS++)) --checkpoint $CHECKPOINT_ID"
echo ""

# 9. Test ALL Mouse Commands
echo -e "${YELLOW}9. Mouse Commands (All Types)${NC}"
echo "=============================="
test_cmd "Mouse move to element" "$CLI mouse move 'button' --checkpoint $CHECKPOINT_ID"
test_cmd "Mouse move-by" "$CLI mouse move-by 50 100 --checkpoint $CHECKPOINT_ID"
test_cmd "Mouse move-to coords" "$CLI mouse move-to 200 300 --checkpoint $CHECKPOINT_ID"
test_cmd "Mouse down" "$CLI mouse down 'body' --checkpoint $CHECKPOINT_ID"
test_cmd "Mouse up" "$CLI mouse up 'body' --checkpoint $CHECKPOINT_ID"
...
test_cmd "Select by index" "$CLI select index 'select#country' 0 $((POS++)) --checkpoint $CHECKPOINT_ID"
test_cmd "Select last" "$CLI select last 'select#country' $((POS++)) --checkpoint $CHECKPOINT_ID"
echo ""
