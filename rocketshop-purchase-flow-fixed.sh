...
# Script to create RocketShop purchase flow test in checkpoint 1680569 - Fixed version
...
# Step 3: Add comment for verification
run_step "Add comment: Verify the home page is displayed" \
    "$BINARY" create-step-comment "$CHECKPOINT_ID" "Verify the home page is displayed" "$POSITION"

echo "📍 Checkpoint 2: Add Product to Bag"
echo "===================================="

# Step 4: Add comment for Border Not Found check
run_step "Add comment: Look for and verify 'Border Not Found' element" \
    "$BINARY" create-step-comment "$CHECKPOINT_ID" "Look for and verify 'Border Not Found' element is present on the page" "$POSITION"
...
# Step 8: Wait for Shopping Bag to display
run_step "Wait for 'Shopping Bag' page/section to display" \
    "$BINARY" create-step-wait-for-element-default "$CHECKPOINT_ID" "Shopping Bag" "$POSITION"
...
# Step 10: Add comment for customer information section
run_step "Add comment: Fill in customer information" \
    "$BINARY" create-step-comment "$CHECKPOINT_ID" "Fill in customer information section" "$POSITION"
...
# Step 11: Add comment for payment information
run_step "Add comment: Fill in payment information" \
    "$BINARY" create-step-comment "$CHECKPOINT_ID" "Fill in payment information" "$POSITION"
...
# Step 14: Add comment for verification
run_step "Add comment: Verify 'Purchase Confirmed!' message is displayed" \
    "$BINARY" create-step-comment "$CHECKPOINT_ID" "Verify 'Purchase Confirmed!' message is displayed" "$POSITION"
...
# Add final comment with expected results
run_step "Add comment: Test Complete - Expected Results" \
    "$BINARY" create-step-comment "$CHECKPOINT_ID" "Test Complete - Expected: Product in bag, checkout completed, payment processed, confirmation displayed and downloadable" "$POSITION"
...
echo ""
echo "Test Objective:"
echo "Verify end-to-end purchase flow from product selection to order confirmation"
echo ""
echo "Prerequisites:"
echo "- Test environment accessible at https://rocketshop.virtuoso.qa"
echo "- Test payment card details available"
echo "- Browser in clean state (no previous sessions)"
echo ""
echo "Expected Results:"
echo "✓ Product added to shopping bag"
echo "✓ Shopping bag displays selected item"
...
echo "- Pass: All steps complete successfully and purchase confirmation received"
echo "- Fail: Any step cannot be completed or expected elements not found"
echo ""
