...
async function loadToolsFromFiles(): Promise<ToolInfo[]> {
  ...
  for (const file of files) {
    ...
    for (const match of toolMatches) {
      ...
      const [, group] = name.match(/virtuoso_([^_]+)/) || [];
...
async function validateTools(tools: ToolInfo[]): Promise<ValidationResult[]> {
  ...
  for (const tool of tools) {
    ...
    if (nameParts.length < 2) {
      errors.push("Tool name must follow pattern: virtuoso_GROUP");
    }
...
async function generateDocumentation(tools: ToolInfo[]): Promise<string> {
